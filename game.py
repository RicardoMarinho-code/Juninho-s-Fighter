...
class Game:
    def __init__(self, screen, assets):
        ...
        # Dados dos personagens (vida, escala, offset)
    ...
    def run(self):
        run = True
        while run:
            self.clock.tick(FPS)  # Controla a taxa de quadros
            ...
            # Verifica se a rodada acabou (alguém morreu)
            if not self.round_over:
                ...
                if pygame.time.get_ticks() - self.round_over_time > ROUND_OVER_COOLDOWN:
                    self.round_over = False
                    self.intro_count = 3
                    # Reinicializa os jogadores
                    self.player_1 = Fighter(1, 200, 380, False, self.WARRIOR_DATA, self.assets['warrior_sheet'], self.WARRIOR_ANIM, self.assets['sword_fx'])
                    self.player_2 = Fighter(2, 700, 380, True, self.WIZARD_DATA, self.assets['wizard_sheet'], self.WIZARD_ANIM, self.assets['magic_fx'])
